...
export interface CICDEditorValidationErrorsProps {
...
export const CICDEditorValidationErrors: React.FC<CICDEditorValidationErrorsProps> = ({
  errors,
  onLineClick,
}) => {
  ...
  if (errors.length === 0) {
    return (
      <div className="rounded-xl border border-success/30 bg-success-light p-4 flex items-center gap-3">
        ...
        <span className="text-sm font-medium text-success">No validation errors — workflow is valid</span>
      </div>
    )
  }
